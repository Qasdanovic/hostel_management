<x-app-layout>
    ...
    <div class="flex justify-between my-7">
        <h1 class="text-white text-3xl">modifier une chambre :</h1>
        <x-button class="bg-red-600">
            <a href="{{ route('chambres.index') }}">annuler la modification</a>
        </x-button>
    </div>
    ...
    <form action="{{ route('chambres.update', $chambre->id) }}" method="post" enctype="multipart/form-data">
        ...
        <div class="flex flex-wrap gap-4">
            ...
            <div class="mt-4 flex-1">
                ...
                <x-text-input id="numero_chambre" class="block mt-1 w-full" type="text" name="numero_chambre" value="{{ $chambre->numero_chambre }}" />
            ...
            <div class="mt-4 flex-1">
                ...
                <select id="type_chambre_id" name="type_chambre_id" class=" bg-gray-900 text-white block mt-1 w-full rounded">
                    ...
                    @foreach ($types as $type)
                        <option value="{{ $type->id }}" {{ $type->id === $chambre->type_chambre_id ? "selected" : '' }}>{{ $type->type_chambre }}</option>
                    @endforeach
                </select>
        ...
        <div class="flex flex-wrap gap-4">
            ...
            <div class="mt-4 flex-1">
                ...
                <x-text-input id="nbr_lits_chambre" class="block mt-1 w-full" type="number" name="nbr_lits_chambre" value="{{ $chambre->nbr_lits_chambre }}" />
                <x-input-error :messages="$errors->get('nbr_lits_chambre')" class="mt-2" />
            </div>
            <div class="mt-4 flex-1">
                <label for="etage_chambre" class="text-white">Étage :</label>
                <x-text-input id="etage_chambre" class="block mt-1 w-full" type="number" name="etage_chambre" value="{{ $chambre->etage_chambre }}" />
            ...
            <div class="mt-4 flex-1">
                ...
                <select id="capacite_id" name="capacite_chambre_id" class=" bg-gray-900 text-white block mt-1 w-full rounded">
                    ...
                    @foreach ($capacites as $capacite)
                        <option value="{{ $capacite->id }}" {{ $capacite->id === $chambre->capacite_chambre_id ? "selected" : '' }}>{{ $capacite->titre_capacite }}</option>
                    @endforeach
                </select>
            ...
            <div class="mt-4 flex-1">
                ...
                <select id="tarif_chambre_id" name="tarif_chambre_id" class="block mt-1 w-full rounded bg-gray-900 text-white">
                    ...
                    @foreach ($tarifs as $tarif)
                        <option value="{{ $tarif->id }}" {{ $tarif->id === $chambre->tarif_chambre_id ? "selected" : '' }}>prix base nuit : {{$tarif->prix_base_nuit}} - prix base passage : {{$tarif->prix_base_passage }}</option>
                    @endforeach
                </select>
        ...
        <div class="flex flex-wrap gap-4">
            ...
            <div class="mt-4 flex-1">
                ...
                <x-text-input id="nombre_adultes_enfants_chambre" class="block mt-1 w-full" type="number" name="nombre_adultes_enfants_chambre" value="{{$chambre->nombre_adultes_enfants_chambre}}" />
            ...
            <div class="mt-4 flex-1">
                ...
                <select id="renfort_chambre" class=" bg-gray-900 text-white mt-1 w-full rounded" name="renfort_chambre">
                    ...
                    <option value="0" {{ 0 === $chambre->renfort_chambre ? "selected" : '' }}>NON</option>
                    <option value="1" {{ 1 === $chambre->renfort_chambre ? "selected" : '' }}>OUI</option>
                </select>
        ...
        <x-button class="bg-blue-600 text-white my-5 m" type='submit'>modifier la chambre</x-button>
    </form>
</x-app-layout>
